...
struct Circle {
	...
	bool isContain(Circle& tar) {
		if (this->radius < tar.radius) return false;
		int _x = (x - tar.x);
		int _y = (y - tar.y);
...
struct CastleNode {
	...
	void findParent(CastleNode* parent_cand) {
		...
		for (list<CastleNode*>::iterator it = parent_child.begin(); it != parent_child.end(); it++) {
...
int main() {
	...
	while (c--) {
		...
		sort(castle.begin() + 1, castle.end());
		// 특수 케이스 처리 
		if (n == 1) {
			cout << 0 << '\n';
